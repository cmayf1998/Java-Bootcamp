...
public class Lecture {
    ...
    public static void getScores() {
        ...
        // int[] scores = {7, 9, 10}; initialized allocation
        int[] scores = new int[3];
        scores[0] = 7;
        scores[1] = 9;
        scores[2] = 10;

        System.out.println("In getScores scores is " + scores);
        for (int i = 0; i < scores.length; i++) {
            System.out.println("index is " + i + " value is " + scores[i]);
            if (i == 1){
                break;
            }
        }
        int[] doubledScores = new int[scores.length];
        for (int i = 0; i < scores.length; i++) {
            doubledScores[i] = scores[i]*2;
        }
