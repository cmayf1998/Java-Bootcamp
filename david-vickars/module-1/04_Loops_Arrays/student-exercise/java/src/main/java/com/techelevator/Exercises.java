...
public class Exercises {
	...
	public boolean firstLast6(int[] nums) {
		/*
		if first is 6 return true - index nums[0]
		if the last is 6 return true - index nums[nums.length-1]
	...
	public int[] demo(int[] nums) {
		int[] valuesDoubled = new int[nums.length];

		for (int i=0; i < nums.length; i++){
			valuesDoubled[i] = nums[i] * 2;
		}
	...
	public int sum3(int[] nums) {
		/*
		declare a variable to hold the sum (int) - initalize to 0
		need a for loop to go through each element
