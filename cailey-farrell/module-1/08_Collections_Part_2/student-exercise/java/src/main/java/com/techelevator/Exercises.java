...
/*
		if (sale == null) {
			return 0.00;
		} if (sale == map.get(itemNumber.toUpperCase())){
	...
	public Map<String, Integer> robPeterToPayPaul(Map<String, Integer> peterPaul) {
		...
		if (peterPaul.get("Peter") > 0 && peterPaul.get("Paul") < 1000) {
			int halfMoney = (int) Math.round(((double) peterPaul.get("Peter") / 2));
			int paulMoney = peterPaul.get("Paul") + (peterPaul.get("Peter") / 2);
			peterPaul.put("Peter", halfMoney);
	...
	public Map<String, Integer> peterPaulPartnership(Map<String, Integer> peterPaul) {
		if (peterPaul.get("Peter") >= 5000 && peterPaul.get("Paul") >= 10000) {
			int peterPartnership = (int) Math.round((double) peterPaul.get("Peter") / 4);
			int paulPartnership = (int) Math.round((double) peterPaul.get("Paul") / 4);
			int partnership = peterPartnership + paulPartnership;

			peterPaul.put("Peter", (peterPaul.get("Peter") - peterPartnership));
			peterPaul.put("Paul", (peterPaul.get("Paul") - paulPartnership));
	...
	public Map<String, String> beginningAndEnding(String[] words) {
		...
		for (String string : words) {
			...
			String last = string.substring(string.length() - 1, string.length());
	...
	public Map<String, Integer> wordCount(String[] words) {
		...
		for (int i = 0; i < words.length; i++) {
			...
			if (map.containsKey(currentKey)) {
				int numberOfKeys = map.get(currentKey) + 1;
				map.put(currentKey, numberOfKeys);
			} else map.put(currentKey, 1); //for keys that only appear once
	...
	public Map<String, Integer> consolidateInventory(Map<String, Integer> mainWarehouse, Map<String, Integer> remoteWarehouse) {
		...
		for (String name : remoteWarehouse.keySet()) {
			int value = remoteWarehouse.get(name);

			if (mainWarehouse.containsKey(name)) {
				mainWarehouse.put(name, mainWarehouse.get(name) + value);
			} else mainWarehouse.put(name, value);
		}
	...
	public List<String> distinctValues(List<String> stringList) {
		Set<String> distinct = new LinkedHashSet<>(stringList);
		stringList.clear();

		stringList.addAll(distinct);
		return stringList;
	}
}
