...
public class Exercises {
	...
	public int[] rotateLeft3(int[] nums) {
		/*
		create new array
		take the element out of nums [0] and place it into the new array last position nums.length-1
		take each element from nums and place it into my new array at position i-1
		return new array
		 */
	...
	public int[] reverse3(int[] nums) {
		...
		for (int i = 0; i < nums.length; i++) {
			reverseArray[nums.length-i-1] = nums[i];
			continue;
		}
		reverseArray[0] = nums[nums.length - 1];
		return reverseArray;
	}
	...
	/*
	 11. Return the number of e en ints in the given array. Note: the % "mod" operator computes the
